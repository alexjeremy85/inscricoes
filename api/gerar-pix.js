...
export default async function handler(req, res) {
    ...
    try {
        ...
        console.log(`💳 Gerando PIX parcela ${parcelaAtual}/${numero_parcelas} [${isProduction ? 'PROD' : 'SANDBOX'}]`);
        ...
        const referenceId = id_inscricao || `inscricao_${Date.now()}`;
        const webhookUrl = process.env.WEBHOOK_URL || 'https://inscricoes-sigma.vercel.app/api/webhook-pagbank';
        ...
        console.log('📤 Enviando PIX para PagBank...');
        ...
        if (!response.ok) {
            console.error('❌ Erro PagBank PIX - Status:', response.status);
